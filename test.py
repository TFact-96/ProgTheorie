...
class Hill_climber:
    ...
    def hill_start(self):
        ...
        for iteration in range(self.amount_of_reset_checks):
            ...
            print(
                f"Random chain {chain_nr}: Nodes pulled: {amt_stab_change_checks * amt_pulls_per_stab_change_check}. Stability: {best_stability}"
            )
            ...
            if not better_stab_found:
                # terminal info
                print(
                    f"No stability change after {amt_stab_change_checks} checks. Saving chain {chain_nr} as local minima...\n"
                )

                # append to local minima list
                local_minimum_chains[copy.copy(best_stability)] = [
